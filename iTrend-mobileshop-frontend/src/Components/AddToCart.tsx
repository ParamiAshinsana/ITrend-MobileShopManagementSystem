...
const AddToCart: React.FC = () => {
  ...
  const handleCheckout = async () => {
    try {
      const userId = localStorage.getItem("userId");  // Assuming the userId is stored in localStorage after sign-in
      if (!userId) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-100 min-h-screen">
      ...
      <div className="text-center mt-6">
        <button onClick={handleCheckout} className="bg-green-500 text-black px-6 py-3 rounded-lg text-lg font-semibold hover:bg-green-600 transition">
          Proceed To Checkout
        </button>
      </div>
    </div>
